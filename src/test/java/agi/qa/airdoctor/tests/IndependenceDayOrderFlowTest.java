...
public class IndependenceDayOrderFlowTest extends BaseTest {
	...
	try {
	  independencedayPage = loginPage.doLogin(prop.getProperty("username"),prop.getProperty("password"));
	  independencedayPage.clearCart();
	  independencedayPage = loginPage.clickShopNow();
	  softAssert = new SoftAssert();
	  int currenttest= independencedayPage.testMe(testContext);
	  Thread.sleep(3000);
	  independencedayPage.selectModel(ModelName, ProductQuantity,ModeltwoName,ProducttwoQuantity); 
	  Thread.sleep(5000);
	  independencedayPage.checkout(email,firstname, lastname, addone, addtwo, cty, state,zipcode,phonenumber); 
	  Map<String, String>  preOrderDetailsMap = independencedayPage.getpreorderdetails();
	  ...
	  independencedayPage.placeOrder();
	  Thread.sleep(15000);
	  independencedayPage.SelectUpsell(Upsell1);
	  Thread.sleep(5000);
	  independencedayPage.getThankYoPageURL();
	  Map<String, String>  productActDetailsMap = independencedayPage.getpostorderdetails();
	  ...
	  System.out.println("=============================================================");
	  independencedayPage.writeexcel(preOrderDetailsMap.get("preordersubtotal"),preOrderDetailsMap.get("preorderShipping"),preOrderDetailsMap.get("preordertax"),preOrderDetailsMap.get("preordertotal"),productActDetailsMap.get("subtotal"),productActDetailsMap.get("Shipping"),productActDetailsMap.get("tax"),productActDetailsMap.get("total"),productActDetailsMap.get("OrderID"),currenttest); 
